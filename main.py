...
def delete_container_for_an_user(user_name, container_id):
    old_data = ''
    ...
    with open('user_data.json') as json_file:
        ...
        for idx, val in enumerate(current_data):
            if val['containerID'] == container_id:
                index_to_delete = idx
        del current_data[index_to_delete]
        old_data[user_name]['containers'] = current_data
...
def show_user_profile(username):
    ...
    if request.method == 'POST':
        if "cont_delete" in request.form:
            delete_container_id = request.form['cont_delete']
            user_name = request.form['uname']
            delete_container_for_an_user(user_name, delete_container_id)
            print("container deleted is", delete_container_id)
        else:
            user_name = request.form['uname']
